...
async function query(url, endpoint, token, request) {
    try {
        console.log("Sending request to:", { url, endpoint, request });
        console.log("Using token:", token);
        const response = await client.post(url, { query: request }, {
            headers: {
        ...
        if (axios.isAxiosError(err)) {
            console.error(`❌ Request failed with status ${err.response?.status}:`, JSON.stringify(err.response?.data));
        }
        else {
            console.error(err);
        }
